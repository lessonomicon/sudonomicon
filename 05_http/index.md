...
```{file="file_server_unsafe.py:do_get"}
...
```{file="file_server_unsafe.py:send_content"}
...
```{file="file_server_unsafe.py:error_page"}
ERROR_PAGE = """\
<html>
...
"""
```
```{file="file_server_unsafe.py:handle_error"}
...
```{file="file_server_unsafe.py:exception"}
...
```{file="file_server_unsafe.py:main"}
...
```{file="nc_localhost.sh"}
nc localhost 8000
```
```{file="nc_allowed.text"}
...
```{file="nc_disallowed.text"}
GET ../motto.txt HTTP/1.1

```
```{file="nc_disallowed.out"}
HTTP/1.0 200 OK
Server: BaseHTTP/0.6 Python/3.12.1
...
```{file="file_server_safe.py:handle_file"}
...
```{file="bird_server_whole.py:main"}
def main():
...
```{file="bird_server_whole.py:server"}
...
```{file="bird_server_whole.py:send"}
    def send_content(self, content, status):
        content = bytes(content, "utf-8")
        self.send_response(int(status))
...
```{file="bird_server_slice.py:filter"}
